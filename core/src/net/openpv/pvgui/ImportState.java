...
public class ImportState extends UIState
{
	String path;
	TextField newValue;
	PVNetwork network;
	public ImportState(String path, PVNetwork network) { this.path = path; this.network = network; }
	@Override
	public void init()
	{
		setupStage();
	...
	public void build(Skin uiSkin)
	{
		...
		dialog.row().pad(8);
		dialog.add(new Label("Import params file", uiSkin)).expandX().fillX().colspan(2);
		dialog.row().pad(8);
	...
	{
		parent.undoable("Imported " + newValue.getText(), network);
		network.readParamsFile(newValue.getText(), parent.getLibrary());
        parent.popState();
	}
