...
public class ExportState extends UIState
{
	String path;
	TextField newValue;
	PVNetwork network;
	public ExportState(String path, PVNetwork network) { this.path = path; this.network = network; }
	@Override
	public void init()
	{
		setupStage();
	...
	public void build(Skin uiSkin)
	{
		...
		dialog.row().pad(8);
		dialog.add(new Label("Export as params file", uiSkin)).expandX().fillX().colspan(2);
		dialog.row().pad(8);
	...
	{
		network.writeParamsFile(newValue.getText());
		parent.message("Exported " + newValue.getText());
        parent.popState();
	}
